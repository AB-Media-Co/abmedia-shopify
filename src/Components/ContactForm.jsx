...
import { Mail, User, Phone, Building, X } from 'lucide-react';
...
const ContactForm = ({
    ...
    const buildTypeOptions = [
        ...
        'Hydrogen + Oxygen (React)', 'Subscription or B2B logic', 'Unsure — need guidance'
    ];
                            ...
                            {step === 1 ? (
                                <div className="space-y-6" >
                                    ...
                                    <div className="space-y-4">
                                        <select name="hasShopifyStore" value={formData.hasShopifyStore === null ? '' : formData.hasShopifyStore.toString()}
                                            onChange={(e) => handleToggleChange(e.target.value === '' ? null : e.target.value === 'true')}
                                            className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-lime-400 focus:border-transparent outline-none transition-all">
                                            <option value="">Do you already have a Shopify store?</option>
                                        ...
                                        <select name="buildType" value={formData.buildTypes[0] || ''}
                                            onChange={(e) => setFormData(prev => ({ ...prev, buildTypes: e.target.value ? [e.target.value] : [] }))}
                                            className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-lime-400 focus:border-transparent outline-none transition-all">
                                            <option value="">Select build type you are interested in?</option>
